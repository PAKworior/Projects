...
private:
	...
	coordinates theme_index;// indexes to the current theme selected
	coordinates pawn_menu_coords;// during pawn promotion it stores the index of the highlited menu 
	coordinates pawn_cords;// stores the index of the pawn that is involved during pawn promotion
	typedef struct Themecolours {// use to colour theme of the game.
		...
		std::string white_boxes = "\x1b[38;5;255m";//
		std::string coordinates = "\x1b[38;5;40m";// those 1-8 and A-H numbers that shows the coordinates of the pieces
		std::string borders = "\x1b[38;5;255m";//border of the chess board default white
	...
	Themecolours colours;// it stores the current ansi theme colours that the program applies to the whole game
	std::string menu_array[3][3][7];// stores the coloured menu option 
	std::string menu_select_array[3][3][7];// when something is selected, game prints the selected thing from this array rather then "menu_array"
	Themecolours menu_colours_array[3][3];// stores all the theme colours
	//chess logic
...
public:
	...
	std::condition_variable main_cv;        // Notifies the main thread that there are pending commands/messages to process (used together with main_mutex).
	std::queue<std::string> main_queue;    // Command queue where worker threads (timer/input) push strings for the main loop to consume; protected by main_mutex.
	std::mutex main_mutex;                 // Protects main_queue and any shared state accessed by main thread and worker threads; used with main_cv.
	std::string pawn_buffer[36];                 // Pre-rendered lines (36) of the pawn-promotion GUI (each element is a line, may include ANSI color codes).
	std::string chessboard_buffer[64];           // Pre-rendered chessboard lines (64). Filled by refresh_chessboard_buffer() and printed by main thread.
	std::string pawn_promotion_menu_buffer[36];  // Pre-rendered pawn-promotion menu lines (36). Used while input_mode == 'p'.
	std::string timer_buffer[23];                // Pre-rendered timer display lines (23). Holds large-digit ASCII art for both players' clocks.
	std::string _notification_buffer[22];        // Display lines (22) for the notification area; updated by _notification_buffer_refresher().
	std::string theme_menu_buffer[36];           // Pre-rendered theme-selection menu lines (36). Used while input_mode == 't'.
	std::atomic<char> input_mode{ 's' }; // Current input mode (atomic because read/written by multiple threads):
	//timer thread
	void timer_thread() {
	...
	chess() {
		...
		menu_colours_array[1][1].attack_spots = "\x1b[38;5;88m";
		menu_colours_array[1][1].safe_spots = "\x1b[38;5;18m";
		...
		menu_colours_array[2][2].black_pieces = "\x1b[38;5;52m"; //rose
		menu_colours_array[2][2].white_pieces = "\x1b[38;5;58m";
		menu_colours_array[2][2].white_boxes = "\x1b[38;5;244m";
		menu_colours_array[2][2].borders = "\x1b[38;5;236m";
		menu_colours_array[2][2].attack_spots = "\x1b[38;5;124m";
	...
	void refresh_pawn_promotion_menu_buffer() {
		for (std::string& s : pawn_promotion_menu_buffer) s = "";
		const std::string& B = colours.borders;
		const std::string& A = colours.attack_spots;
		const std::string& S = colours.safe_spots;
		const std::string& C = colours.coordinates;
		std::string P = (pawn_cords.y == 0) ? colours.white_pieces : colours.black_pieces;
		std::string left_borders[7] = {
			B + "|" + A + "##" + B + "|",
			B + "|" + A + "##" + B + "|",
			B + "|\\" + A + "#" + B + "|",
			B + "|" + S + "#" + B + "\\|",
		...
		std::string left_select_border[18] = {
			"       /|",
			"      /" + C + "#" + B + "|",
			"     /" + C + "#" + B + "/\\",
			"    /" + C + "#" + B + "/\\/",
		...
		std::string right_select_border[18] = {
			"|\\       ",
			"|" + C + "#" + B + "\\      ",
			"/\\" + C + "#" + B + "\\     ",
			"\\/\\" + C + "#" + B + "\\    ",
		...
		std::string padding = "                      ";
		int offset = 18;
		if (!pawn_menu_coords.x && !pawn_menu_coords.y) {
			pawn_promotion_menu_buffer[0] += left_select_border[0] + "                      " + right_select_border[0];
			pawn_promotion_menu_buffer[1] += left_select_border[1] + "                      " + right_select_border[1];
			pawn_promotion_menu_buffer[2] += left_select_border[2] + "                      " + right_select_border[2];
			pawn_promotion_menu_buffer[3] += left_select_border[3] + "                      " + right_select_border[3];
			pawn_promotion_menu_buffer[4] += left_select_border[4] + " _   _____________  _ " + right_select_border[4];
			pawn_promotion_menu_buffer[5] += left_select_border[5] + "|" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|" + right_select_border[5];

			pawn_promotion_menu_buffer[13] += "   \\ " + C + "#" + B + "  \\   \\____Queen_____/   /  " + C + "#" + B + " /   ";
			pawn_promotion_menu_buffer[14] += "    \\ " + C + "#" + B + " |                      | " + C + "#" + B + " /    ";
			pawn_promotion_menu_buffer[15] += "     \\ " + C + "#" + B + "|                      |" + C + "#" + B + " /     ";
			pawn_promotion_menu_buffer[16] += "      \\" + C + "#" + B + "|                      |" + C + "#" + B + "/      ";
			pawn_promotion_menu_buffer[17] += "       \\|                      |/       ";
		}
		else {
			pawn_promotion_menu_buffer[0] += "                                        ";
			pawn_promotion_menu_buffer[1] += "                                        ";
			pawn_promotion_menu_buffer[2] += "                                        ";
			pawn_promotion_menu_buffer[3] += "                                        ";
			pawn_promotion_menu_buffer[4] += "          _   _____________  _          ";
			pawn_promotion_menu_buffer[5] += "         |" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|         ";

			pawn_promotion_menu_buffer[13] += "            \\______________/            ";
			pawn_promotion_menu_buffer[14] += "                                        ";
			pawn_promotion_menu_buffer[15] += "                                        ";
			pawn_promotion_menu_buffer[16] += "                                        ";
			pawn_promotion_menu_buffer[17] += "                                        ";
		}
		if (!pawn_menu_coords.x && pawn_menu_coords.y) {
			pawn_promotion_menu_buffer[0 + offset] += left_select_border[0] + "                      " + right_select_border[0];
			pawn_promotion_menu_buffer[1 + offset] += left_select_border[1] + "                      " + right_select_border[1];
			pawn_promotion_menu_buffer[2 + offset] += left_select_border[2] + "                      " + right_select_border[2];
			pawn_promotion_menu_buffer[3 + offset] += left_select_border[3] + "                      " + right_select_border[3];
			pawn_promotion_menu_buffer[4 + offset] += left_select_border[4] + " _   _____________  _ " + right_select_border[4];
			pawn_promotion_menu_buffer[5 + offset] += left_select_border[5] + "|" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|" + right_select_border[5];

			pawn_promotion_menu_buffer[13 + offset] += "   \\ " + C + "#" + B + "  \\   \\_____Rook_____/   /  " + C + "#" + B + " /   ";
			pawn_promotion_menu_buffer[14 + offset] += "    \\ " + C + "#" + B + " |                      | " + C + "#" + B + " /    ";
			pawn_promotion_menu_buffer[15 + offset] += "     \\ " + C + "#" + B + "|                      |" + C + "#" + B + " /     ";
			pawn_promotion_menu_buffer[16 + offset] += "      \\" + C + "#" + B + "|                      |" + C + "#" + B + "/      ";
			pawn_promotion_menu_buffer[17 + offset] += "       \\|                      |/       ";
		}
		else {
			pawn_promotion_menu_buffer[0 + offset] += "                                        ";
			pawn_promotion_menu_buffer[1 + offset] += "                                        ";
			pawn_promotion_menu_buffer[2 + offset] += "                                        ";
			pawn_promotion_menu_buffer[3 + offset] += "                                        ";
			pawn_promotion_menu_buffer[4 + offset] += "          _   _____________  _          ";
			pawn_promotion_menu_buffer[5 + offset] += "         |" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|         ";

			pawn_promotion_menu_buffer[13 + offset] += "            \\______________/            ";
			pawn_promotion_menu_buffer[14 + offset] += "                                        ";
			pawn_promotion_menu_buffer[15 + offset] += "                                        ";
			pawn_promotion_menu_buffer[16 + offset] += "                                        ";
			pawn_promotion_menu_buffer[17 + offset] += "                                        ";
		}
		if (pawn_menu_coords.x && !pawn_menu_coords.y) {
			pawn_promotion_menu_buffer[0] += padding + left_select_border[0] + "                      " + right_select_border[0];
			pawn_promotion_menu_buffer[1] += padding + left_select_border[1] + "                      " + right_select_border[1];
			pawn_promotion_menu_buffer[2] += padding + left_select_border[2] + "                      " + right_select_border[2];
			pawn_promotion_menu_buffer[3] += padding + left_select_border[3] + "                      " + right_select_border[3];
			pawn_promotion_menu_buffer[4] += padding + left_select_border[4] + " _   _____________  _ " + right_select_border[4];
			pawn_promotion_menu_buffer[5] += padding + left_select_border[5] + "|" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|" + right_select_border[5];

			pawn_promotion_menu_buffer[13] += padding + left_select_border[13] + "   \\____Knight____/   " + right_select_border[13];
			pawn_promotion_menu_buffer[14] += padding + left_select_border[14] + "                      " + right_select_border[14];
			pawn_promotion_menu_buffer[15] += padding + left_select_border[15] + "                      " + right_select_border[15];
			pawn_promotion_menu_buffer[16] += padding + left_select_border[16] + "                      " + right_select_border[16];
			pawn_promotion_menu_buffer[17] += padding + left_select_border[17] + "                      " + right_select_border[17];
		}
		else {
			pawn_promotion_menu_buffer[0] += padding + "                                        ";
			pawn_promotion_menu_buffer[1] += padding + "                                        ";
			pawn_promotion_menu_buffer[2] += padding + "                                        ";
			pawn_promotion_menu_buffer[3] += padding + "                                        ";
			pawn_promotion_menu_buffer[4] += padding + "          _   _____________  _          ";
			pawn_promotion_menu_buffer[5] += padding + "         |" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|         ";

			pawn_promotion_menu_buffer[13] += padding + "            \\______________/            ";
			pawn_promotion_menu_buffer[14] += padding + "                                        ";
			pawn_promotion_menu_buffer[15] += padding + "                                        ";
			pawn_promotion_menu_buffer[16] += padding + "                                        ";
			pawn_promotion_menu_buffer[17] += padding + "                                        ";
		}
		if (pawn_menu_coords.x && pawn_menu_coords.y) {
			pawn_promotion_menu_buffer[0 + offset] += padding + left_select_border[0] + "                      " + right_select_border[0];
			pawn_promotion_menu_buffer[1 + offset] += padding + left_select_border[1] + "                      " + right_select_border[1];
			pawn_promotion_menu_buffer[2 + offset] += padding + left_select_border[2] + "                      " + right_select_border[2];
			pawn_promotion_menu_buffer[3 + offset] += padding + left_select_border[3] + "                      " + right_select_border[3];
			pawn_promotion_menu_buffer[4 + offset] += padding + left_select_border[4] + " _   _____________  _ " + right_select_border[4];
			pawn_promotion_menu_buffer[5 + offset] += padding + left_select_border[5] + "|" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|" + right_select_border[5];

			pawn_promotion_menu_buffer[13 + offset] += padding + left_select_border[13] + "   \\____Bishop____/   " + right_select_border[13];
			pawn_promotion_menu_buffer[14 + offset] += padding + left_select_border[14] + "                      " + right_select_border[14];
			pawn_promotion_menu_buffer[15 + offset] += padding + left_select_border[15] + "                      " + right_select_border[15];
			pawn_promotion_menu_buffer[16 + offset] += padding + left_select_border[16] + "                      " + right_select_border[16];
			pawn_promotion_menu_buffer[17 + offset] += padding + left_select_border[17] + "                      " + right_select_border[17];
		}
		else {
			pawn_promotion_menu_buffer[0 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[1 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[2 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[3 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[4 + offset] += padding + "          _   _____________  _          ";
			pawn_promotion_menu_buffer[5 + offset] += padding + "         |" + A + "#" + B + "\\/______________\\/" + A + "#" + B + "|         ";

			pawn_promotion_menu_buffer[13 + offset] += padding + "            \\______________/            ";
			pawn_promotion_menu_buffer[14 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[15 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[16 + offset] += padding + "                                        ";
			pawn_promotion_menu_buffer[17 + offset] += padding + "                                        ";
		}
		int index = 0;
		int index2 = 0;

		for (int y = 0; y < 7; y++) {
			if (!pawn_menu_coords.x && !pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6] += left_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6] += "         ";
			pawn_promotion_menu_buffer[y + 6] += left_borders[y] + colours.white_boxes;
			std::string cur_colour = colours.white_boxes;
			for (int x = 0; x < 14; x++, index++) {
				char ch = data("Queen", index);
				if (ch == ' ') {
					if (cur_colour != colours.white_boxes) {
						cur_colour = colours.white_boxes;
						pawn_promotion_menu_buffer[y + 6] += colours.white_boxes;
					}
					pawn_promotion_menu_buffer[y + 6] += "#";
				}
				else {
					if (cur_colour != P) {
						cur_colour = P;
						pawn_promotion_menu_buffer[y + 6] += P;
					}
					pawn_promotion_menu_buffer[y + 6] += ch;
				}
			}
			pawn_promotion_menu_buffer[y + 6] += right_borders[y];
			if (!pawn_menu_coords.x && !pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6] += right_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6] += "         ";
			pawn_promotion_menu_buffer[y + 6] += padding;
			if (pawn_menu_coords.x && !pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6] += left_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6] += "         ";
			pawn_promotion_menu_buffer[y + 6] += left_borders[y] + colours.white_boxes;
			cur_colour = colours.white_boxes;
			for (int x = 0; x < 14; x++, index2++) {
				char ch = data("knight", index2);
				if (ch == ' ') {
					if (cur_colour != colours.white_boxes) {
						cur_colour = colours.white_boxes;
						pawn_promotion_menu_buffer[y + 6] += colours.white_boxes;
		...
		index2 = 0;
		for (int y = 0; y < 7; y++) {
			if (!pawn_menu_coords.x && pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6 + offset] += left_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6 + offset] += "         ";
			pawn_promotion_menu_buffer[y + 6 + offset] += left_borders[y] + colours.white_boxes;
			std::string cur_colour = colours.white_boxes;
			for (int x = 0; x < 14; x++, index++) {
				char ch = data("Rook", index);
				if (ch == ' ') {
					if (cur_colour != colours.white_boxes) {
						cur_colour = colours.white_boxes;
						pawn_promotion_menu_buffer[y + 6 + offset] += colours.white_boxes;
					}
					pawn_promotion_menu_buffer[y + 6 + offset] += "#";
				}
				else {
					if (cur_colour != P) {
						cur_colour = P;
						pawn_promotion_menu_buffer[y + 6 + offset] += P;
					}
					pawn_promotion_menu_buffer[y + 6 + offset] += ch;
				}
			}
			pawn_promotion_menu_buffer[y + 6 + offset] += right_borders[y];
			if (!pawn_menu_coords.x && pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6 + offset] += right_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6 + offset] += "         ";
			pawn_promotion_menu_buffer[y + 6 + offset] += padding;
			if (pawn_menu_coords.x && pawn_menu_coords.y)pawn_promotion_menu_buffer[y + 6 + offset] += left_select_border[y + 6]; else pawn_promotion_menu_buffer[y + 6 + offset] += "         ";
			pawn_promotion_menu_buffer[y + 6 + offset] += left_borders[y] + colours.white_boxes;
			cur_colour = colours.white_boxes;
			for (int x = 0; x < 14; x++, index2++) {
				char ch = data("Bishop", index2);
				if (ch == ' ') {
					if (cur_colour != colours.white_boxes) {
						cur_colour = colours.white_boxes;
						pawn_promotion_menu_buffer[y + 6 + offset] += colours.white_boxes;
	...
	void comand_executer(std::string comand) {
		...
		std::regex pattern11(R"(\s*-c\s+([^\s]+)\s+([^\s]+))");// changes the input mode to theme
		...
		if (std::regex_match(comand, matches, pattern1)) {
			...
			if (valid[x2][y2] == 1) {// this condition checks if he is doing a valid move
				int last_ch = board[x1][y1][board[x1][y1].length() - 1];
				if (last_ch == 'k' || last_ch == 'g') {//to know if rook or king moved because if it moved, it means, they can't castle
					if (last_ch == 'g') {//king
						if (board[x1][y1][0] == 'B') {
							black_right_rook_can_castel = false;
							black_left_rook_can_castel = false;
				...
				board[x1][y1] = "null";
				clear(valid);
				if (last_ch == 'r' && (y2 == 0 || y2 == 7)) {
					input_mode = 'p';
					pawn_cords.x = x2;
					pawn_cords.y = y2;
					{
						std::lock_guard<std::mutex> lock(main_mutex);
						main_queue.push("-m 4000 \"Use arrow keys to navigate and  enter to select\" ");
					}
				}
			}
		}
			...
			timer_queue.push(n);
			timer_mutex.unlock();
			_notification_buffer_refresher();
		}
			...
			if (input_mode == 't') {
				...
				refresh_theme_buffer();
			}
			else if (input_mode == 'p') {
				if (ch == "enter") input_mode = 's';
				else if (ch == "up") pawn_menu_coords.y--;
				else if (ch == "down")pawn_menu_coords.y++;
				else if (ch == "left")pawn_menu_coords.x--;
				else if (ch == "right")pawn_menu_coords.x++;
				if (pawn_menu_coords.y == -1)pawn_menu_coords.y = 1;
				if (pawn_menu_coords.y == 2)pawn_menu_coords.y = 0;
				if (pawn_menu_coords.x == -1) pawn_menu_coords.x = 1;
				if (pawn_menu_coords.x == 2) pawn_menu_coords.x = 0;

				if (pawn_menu_coords.x == 0 && pawn_menu_coords.y == 0) {
					if (pawn_cords.y == 7) board[pawn_cords.x][pawn_cords.y] = "BlackQueen"; else board[pawn_cords.x][pawn_cords.y] = "WhiteQueen";
				}
				else if (pawn_menu_coords.x == 1 && pawn_menu_coords.y == 0) {
					if (pawn_cords.y == 7) board[pawn_cords.x][pawn_cords.y] = "BlackKnight"; else board[pawn_cords.x][pawn_cords.y] = "WhiteKnight";
				}
				else if (pawn_menu_coords.x == 0 && pawn_menu_coords.y == 1) {
					if (pawn_cords.y == 7) board[pawn_cords.x][pawn_cords.y] = "BlackRook"; else board[pawn_cords.x][pawn_cords.y] = "WhiteRook";
				}
				else if (pawn_menu_coords.x == 1 && pawn_menu_coords.y == 1) {
					if (pawn_cords.y == 7) board[pawn_cords.x][pawn_cords.y] = "BlackBishop"; else board[pawn_cords.x][pawn_cords.y] = "WhiteBishop";
				}
			}
		}
...
int main() {
	...
	ShowWindow(GetConsoleWindow(), SW_MAXIMIZE);// makes the window fullscreen
	chess Chess;
	std::thread timer_thread(&chess::timer_thread, &Chess);// runs the scheduler
	std::ios::sync_with_stdio(false);// to make i/o operations faster
	std::cin.tie(nullptr);			 // to make i/o operations faster
	//just getting the main thread id displayed for testing purposes
	{
	...
	//initial buffer refresh i will refactor it to look nicer later
	Chess.refresh_chessboard_buffer();
	Chess.refresh_timer_buffer();
	Chess.refresh_theme_buffer();
	Chess.refresh_pawn_promotion_menu_buffer();
	//Chess.refresh_theme_buffer();
	for (int i = 0; i <= 63; i++) {
	...
	std::thread input_thread(&chess::input, &Chess);// runs the input thread/shell if you can't read it
	std::vector<std::string> commands;// to store the commands received from main_queue
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);// to manipulate the console cursor position
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	std::string frame;// to store the whole frame before printing it to avoid flickering
	frame.reserve(64 * 200);// just allocating some memory to avoid reallocation during the game loop
	...
	while (Chess.running) {
		...
		SetConsoleCursorPosition(hConsole, { 0,0 });
		for (int i = 0; i <= 63; i++) {
			frame += Chess.chessboard_buffer[i];
			if (i >= 0 && i <= 21) {
				frame += Chess.timer_buffer[i]; frame += "  "; frame += Chess._notification_buffer[i];
			}
			else if (i < 58) {
				if (Chess.input_mode == 't')frame += Chess.theme_menu_buffer[i - 22];
				else if (Chess.input_mode == 'p')frame += Chess.pawn_promotion_menu_buffer[i - 22];
